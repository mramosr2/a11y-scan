...
app.get("/", (_req, res) => {
  res
    ...
    .send({ ok: true, message: "A11y Site Scanner ready. POST /scan { url }" });
});
...
app.get("/privacy", (_req, res) => {
  // First try repo-root privacy.html 
  const rootPrivacy = path.join(__dirname, "privacy.html");
  // If not present, fall back to /public/privacy.html
  const publicPrivacy = path.join(__dirname, "public", "privacy.html");
  const fileToServe = fs.existsSync(rootPrivacy) ? rootPrivacy : publicPrivacy;

  if (!fs.existsSync(fileToServe)) {
    return res.status(404).type("text/plain").send("privacy.html not found");
  }
...
function wcagFromTags(tags = [], help = "") {
  ...
  const name = help || "";
  const label = code
    ? `WCAG 2.1 — ${code} ${name} (${level})`
    : `WCAG 2.1 — ${name || "Unmapped"} (${level})`;
...
app.post("/scan", async (req, res) => {
  ...
  if (!url || typeof url !== "string") {
    return res
      .status(400)
      .json({ error: "missing_url", message: "Body must be { url: string }" });
  }
  ...
  try {
    ...
    const result = {
      ...
      wcag_findings,
      contrast_checks: [], // leave empty unless we compute real ratios
      image_metadata: null,
